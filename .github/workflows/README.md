# AI Code Review - GitHub Actions 

这个项目使用 GitHub Actions 和 Claude AI 自动审查 Pull Requests 中的代码变更。

## 功能特性

- 🤖 **自动 AI 审查**: PR 创建或更新时自动触发
- 💬 **PR 评论**: 审查结果直接作为评论发布到 PR
- 🔍 **智能分析**: 使用 Claude 3.5 Sonnet 分析代码变更
- 📁 **多语言支持**: 支持 TypeScript, JavaScript, Python, Go, Rust, Java, C/C++ 等
- ⚙️ **可配置**: 可自定义排除模式和文件限制

## 设置步骤

### 1. 获取 Anthropic API Key

1. 访问 [Anthropic Console](https://console.anthropic.com/)
2. 创建 API Key
3. 复制 API Key

### 2. 配置 GitHub Secrets

在你的 GitHub 仓库中添加 Secret:

1. 进入仓库的 **Settings** → **Secrets and variables** → **Actions**
2. 点击 **New repository secret**
3. 添加以下 secret:

| Secret 名称 | 值 |
|------------|-----|
| `ANTHROPIC_API_KEY` | 你的 Anthropic API Key |

### 3. 提交工作流文件

确保以下文件已提交到仓库:

```
.github/
├── workflows/
│   └── code-review.yml    # GitHub Actions 工作流
└── scripts/
    └── code-review.js     # AI 审查脚本
```

### 4. 安装依赖

```bash
npm install
```

## 使用方法

创建或更新 Pull Request 后，AI 代码审查会自动运行:

1. **触发条件**:
   - PR 打开时
   - PR 有新的 commit 时
   - PR 重新打开时

2. **查看审查结果**:
   - 进入 PR 页面
   - 滚动到评论区域
   - 查看 "🤖 AI Code Review" 评论

## 工作流程

```
┌─────────────────┐
│  PR 创建/更新   │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ GitHub Actions  │
│   触发工作流    │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  获取 PR diff   │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ 发送到 Claude   │
│   AI 审查       │
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  发布评论到 PR  │
└─────────────────┘
```

## 配置选项

在 `.github/scripts/code-review.js` 中可以调整配置:

```javascript
const CONFIG = {
  maxFiles: 10,              // 最多审查的文件数
  maxFileSize: 3000,         // 单个文件最大字符数
  excludePatterns: [         // 排除的文件模式
    'node_modules',
    'dist',
    'build',
    '*.min.js',
    // ...
  ]
};
```

## 审查报告格式

AI 审查结果包含以下部分:

1. **Summary**: 变更概述
2. **Issues**: 发现的问题、bug 或安全隐患
3. **Suggestions**: 改进建议和最佳实践
4. **Positive**: 做得好的地方

## 示例审查评论

```markdown
## 🤖 AI Code Review

### Summary
This PR adds user authentication functionality...

### Issues
- `app/auth/login.tsx:45`: Potential security issue - password should be hashed before comparison
- `lib/db.ts:12`: SQL injection vulnerability - use parameterized queries

### Suggestions
- Consider adding error handling for failed login attempts
- Add rate limiting to prevent brute force attacks
- Use environment variables for sensitive configuration

### Positive
- Clean code structure
- Good TypeScript usage
- Comprehensive error messages

---
*This review was generated by [Claude](https://www.anthropic.com/claude)*
```

## 故障排除

### 工作流未触发

- 检查 PR 是否包含支持的文件类型
- 确认工作流文件在 `.github/workflows/` 目录下
- 查看 Actions 标签页查看错误信息

### API 错误

- 验证 `ANTHROPIC_API_KEY` secret 是否正确设置
- 检查 API Key 是否有足够的配额

### 审查未发布

- 确认 GitHub Token 有 `pull-requests: write` 权限
- 查看 Actions 日志获取详细错误信息

## 成本说明

- Claude API 按使用量计费
- 每个 PR 审查大约消耗 1000-5000 tokens
- 建议设置使用限制以控制成本

## 许可证

MIT
